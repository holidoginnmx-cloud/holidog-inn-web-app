...
export async function crearClienteYPerro(
  ...
  const parsed = nuevoPerroSchema.safeParse(leerFormData(formData));
  if (!parsed.success) {
    return { ok: false, error: parsed.error.issues[0]?.message ?? "Datos inválidos" };
  }
  const { cliente, perro } = parsed.data;

  let supabase: DB;
  try {
    supabase = createSupabaseServerClient();
  } catch (e) {
    console.error("[perros] Supabase no configurado:", e);
...
export async function actualizarClienteYPerro(
  ...
  const { error: clienteErr } = await supabase.from("clientes").update(cliente).eq("id", clienteId);
  if (clienteErr) {
